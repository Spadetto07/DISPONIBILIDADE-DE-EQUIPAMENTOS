...
if aba == "Equipamentos Utilizados":
    ...
    st.write(f"Marque os equipamentos [{regime_pincel}]")
    ...
    if st.button("GERAR RELATÓRIO WHATSAPP"):
        ...
        for tit, chave in [("(24 horas)", "24h"), ("(12 horas)", "12h"), ("(ADM)", "ADM"), ("(EVENTUAL)", "EV")]:
            maquinas_reg = [m for m, r in st.session_state.maquinas_regime.items() if r == chave]
            if maquinas_reg:
                txt += f"{tit}\n"
                for e in sorted(maquinas_reg): txt += f"✅ {formatar_prefixo(e)} CASP\n"
                txt += "\n"
